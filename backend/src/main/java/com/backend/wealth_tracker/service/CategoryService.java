...
public class CategoryService {
    ...
    public Category saveCategory(CreateCategoryDTO createCategoryDTO, String userName) throws ResourceNotFoundException, ResourceAlreadyExistsException, UnAuthorizedException {
        User user = this.authService.getUserByUsername(userName);
        if (!Helper.isProfileIdValid(user.getProfiles(), createCategoryDTO.getProfileId())) {
            LOGGER.atError().log("Category to be saved had illegal profile ID: {}", createCategoryDTO.getProfileId());
    ...
    public Category updateCategory(UpdateCategoryDTO updateCategoryDTO, String userName) throws ResourceNotFoundException, ResourceAlreadyExistsException {
        User user = this.authService.getUserByUsername(userName);
        Optional<Category> categoryOptional = this.categoryRepository.findByIdAndUserId(updateCategoryDTO.getId(), user.getId());
        ...
        Category category = updateCategoryValues(updateCategoryDTO, categoryOptional.get(), user.getId());
        Category updatedCategory = this.categoryRepository.save(category);
        LOGGER.atInfo().log("Category updated : {}", updatedCategory);
        return updatedCategory;
    }

    private Category updateCategoryValues(UpdateCategoryDTO updateCategoryDTO, Category category, Long userId) {
    ...
    public void deleteCategory(Long id, String userName) throws ResourceNotFoundException {
        User user = this.authService.getUserByUsername(userName);
        Optional<Category> categoryOptional = this.categoryRepository.findByIdAndUserId(id, user.getId());
        if (categoryOptional.isEmpty()) {
