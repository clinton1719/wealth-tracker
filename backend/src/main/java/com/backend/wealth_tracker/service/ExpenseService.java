...
public class ExpenseService {
  ...
  public Expense updateExpense(UpdateExpenseDTO updateExpenseDTO) throws ResourceNotFoundException {
    ...
    if (updateExpenseDTO.getCategoryId() != null) {
      Optional<Category> categoryOptional =
          this.categoryRepository.findById(updateExpenseDTO.getCategoryId());
      if (categoryOptional.isEmpty()) {
        throw new ResourceNotFoundException(
            "Category not found for id: " + updateExpenseDTO.getCategoryId());
      }
    ...
    if (updateExpenseDTO.getProfileId() != null) {
      Optional<Profile> optionalProfile =
          this.profileRepository.findById(updateExpenseDTO.getProfileId());
      if (optionalProfile.isEmpty()) {
        throw new ResourceNotFoundException(
            "Profile not found for ID: " + updateExpenseDTO.getProfileId());
      }
      expense.setProfile(optionalProfile.get());
    }
    if (updateExpenseDTO.getAccountId() != null) {
      Optional<Account> optionalAccount =
          this.accountRepository.findById(updateExpenseDTO.getAccountId());
      if (optionalAccount.isEmpty()) {
        throw new ResourceNotFoundException(
            "Account not found for ID: " + updateExpenseDTO.getAccountId());
      }
      expense.setAccount(optionalAccount.get());
    }
