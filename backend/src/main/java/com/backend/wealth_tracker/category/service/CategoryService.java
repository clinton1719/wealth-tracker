package com.backend.wealth_tracker.category.service;
  ...
  public Category saveCategory(CreateCategoryDTO createCategoryDTO, String userName)
    ...
    Profile profile =
        user.getProfiles().parallelStream()
            .filter(p -> Objects.equals(p.getProfileId(), createCategoryDTO.getProfileId()))
            .findFirst()
            .orElseThrow(
                () -> {
                  LOGGER
                      .atError()
                      .log("Profile id not found while saving category: {}", createCategoryDTO);
