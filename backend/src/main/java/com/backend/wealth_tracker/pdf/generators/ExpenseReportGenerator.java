...
public class ExpenseReportGenerator
    ...
    protected ExpenseReportModel loadData(ExpenseReportRequest expenseReportRequest) {
        List<ExpenseReportSummaryProjection> expenseReportSummaryProjectionList =
                expenseService.getExpensesWithNamesInRange(expenseReportRequest.startDate(), expenseReportRequest.endDate());
        ...
        ExpenseReportModel expenseReportModel =
                new ExpenseReportModel(
                        expenseReportSummaryProjectionList, totalExpensesFormatted, expenseReportRequest.startDate(), expenseReportRequest.endDate(), chartImagesMap);
        LOGGER.info(
    ...
    protected String renderHtml(ExpenseReportModel model) throws PdfGenerationException {
        ...
        html =
                html.replace("{{ROWS}}", rows.toString())
                        ...
                        .replace("{{categoryTable}}", Base64.getEncoder().encodeToString(model.chartImagesMap().get("categoryTable")))
                        .replace("{{tagTable}}", Base64.getEncoder().encodeToString(model.chartImagesMap().get("tagTable")))
                        .replace("{{categoryPie}}", Base64.getEncoder().encodeToString(model.chartImagesMap().get("categoryPie")))
                        .replace("{{tagPie}}", Base64.getEncoder().encodeToString(model.chartImagesMap().get("tagPie")))
                        .replace("{{categoryLine}}", Base64.getEncoder().encodeToString(model.chartImagesMap().get("categoryLine")))
                        .replace("{{tagLine}}", Base64.getEncoder().encodeToString(model.chartImagesMap().get("tagLine")))
                        .replace("{{expenseLine}}", Base64.getEncoder().encodeToString(model.chartImagesMap().get("expenseLine")))
